...
\paragraph{Sui risultati della first order perturbation theory} Riportiamo le ultime considerazioni sui termini di multipolo ottenuti nella lezione precedente.\\
...
\paragraph{Stima del dipolo} Consideriamo adesso i termini di dipolo. Poiché $k< 1$ MeV (spesso si prende dell'ordine del keV) si ha che $\mean{\vec{k}\cdot\vec{r}_i}/\hbar c\simeq 10/200 = 1/20\ll 1$, quindi è ragionevole fare la \textbf{Long Wavelength Approximation}\index{long wavelength approximation} per cui sfruttando la $\delta^3(\vec{x}-\vec{r}_i)$ che compare nell'espressione della corrente possiamo approssimare $\exp{(i\vec{k}\cdot\vec{x}_i)}\sim 1$ nel calcolo dell'elemento di matrice, per cui\footnote{Da qui in poi chiameremo il versore di polarizzazione $\widehat{\varepsilon}$ con $\hat{e}$ per semplicità di scrittura.}:
...
Facciamo l'ipotesi di long wavelength approximation\index{long wavelength approximation}, per cui:
...
\paragraph{Regole di selezione} Poiché $\vec{J}_i = \vec{J}_f + \vec{J}$ ho che $\pi_i = \pi_f \cdot \pi(T_J)$, per cui non tutti i multipoli sono ammessi; infatti $|J_i - J_f|\leq J\leq J_i+J_f$, allora:
\begin{itemize}
...
Come mostrato nello schema in Figura \ref{0310_bande}, abbiamo 3 possibilità di decadimento:
\begin{itemize}
...
\end{itemize}
Per quanto riguarda le stime di Weisskopf\index{stime di Weisskopf}, ovviamente non saranno precise perché queste sono bande rotazionali\index{bande rotazionali}, quindi non è possibile fare l'approssimazione di simmetria sferica; tuttavia ci danno l'ordine di grandezza:
\begin{displaymath}
\begin{aligned}
