...
\subsection{LUNA}\esperimento{LUNA}
...
\begin{enumerate}
	\item LUNA I\footnote{Fu usato un acceleratore \textit{\vir{home made}}, progetto di laurea di uno studente, che cosisteva di un Van de Graaff a 50 kV. Il fascio aveva uno \textit{spread} $<20$ eV (molto piccolo) che peremtteva la determinanzione con accuratezza dell'energia delle particelle incidenti e inoltre il voltaggio era noto meglio di 1 parte su $\ord{-4}$.} dal 1994 al 2003.
	\item LUNA II dal 2000 al 2020.
	\item LUNA MV in progettazione.
\end{enumerate}

\subsubsection{LUNA I}\esperimento{LUNA!LUNA I}
\paragraph{La prima reazione} Nell'esperimento LUNA I si studiò per prima la reazione:
$$\ce{^3He} + \ce{^3He} \to \ce{^4He} + 2p$$
dove un $\ce{^3He}$ cosisteva in un fascio con una corrente pari a 300 $\mu$A e l'altro in una targhetta gassosa posta nella camera di interazione a una pressione\footnote{Da una versione a quella successiva si è migliorato anche questo settaggio.} di $0.5$ mbar. Il fascio che non faceva reazione veniva portato all'interno di questa camera provocando un aumento della temperatura (\textit{beam dump}\index{beam dump@\textit{beam dump}}) che poteva essere misurato con grande accuratezza e dal quale si otteneva la \textit{beam current} con una precisione del 3\%.\\
\complementi{l'ipotesi di Fowler} %
\noindent Fu scelta questa reazione non solo perché presente nella $pp$I\index{Catena protone-protone@Catena protone-protone $pp$!e@$pp$I}, ma anche perché negli anni '90 era particolarmente discussa la questione dei neutrini solari e Fowler aveva supposto che la sezione d'urto di tale reazione fosse sottostimata a causa di una risonanza non osservata\footnote{Guarda la sezione \secrif{0325-sec-risnu} per l'ipotesi di Fowler.}. Prima di LUNA non si avevano acquisizioni per energie inferiori ai 26 keV e il picco di Gamow di tale reazione è $21 \pm 5$ keV; con LUNA I si raggiunsero $16.5$ keV misurando\footnote{Si avevano 2 eventi al mese.} una sezione d'urto pari a 20 fb$= 2\cdot \ord{-38}\unit{cm}^2$. Il rivelatore (composto da quattro\footnote{Con $\Delta E$ si intende la perdita di energia nell'attraversamento del mezzo.%! Credo ci si riferisca proprio al current bump
} $\Delta E$-$E$ \textit{particle detector}) permetteva di valutare evento per evento le regioni in cui osservarlo, questo per poter stimare l'\textit{induced background}\index{ZZBackground@\textbf{Background}!induced@\textit{induced}}, ovvero il fondo indotto dalla misura stessa; un esempio di tale fenomeno è la reazione $\ce{^3He}(d,p)\ce{^4He}$ dovuta alla contaminazione (sempre presente) del fascio di deuterio. In Figura \ref{0415_DEE} riportiamo i risultati della presa dati: si possono facilmente notare 2 regioni principali. 
...
Per energie inferiori ai $20.7$ keV le 2 regioni si sovrappongono (perdita di eventi), allora furono installati 2 rivelatori di protoni in successione, messi in coincidenza dal momento che la reazione $\ce{^3He}(d,p)\ce{^4He}$ produce un solo protone e non due come per quella di interesse. Per avere un'idea di quanto fu ridotto il fondo si guardi la Figura \ref{0415_r1r2}.

\begin{figure}[!h]
	\centering
	\includegraphics[scale=0.5]{Immagini/0415_riv12.png}
	\caption{Riduzione del fondo, sul piano $x-y$ vi è la matrice $\Delta E$-$E$, mentre sull'asse $z$ sono riportati i conteggi: in alto risultati per un solo rivelatore a terra (presenza anche di raggi cosmici), in basso 2 rivelatori in coincidenza nel sottosuolo.}
	\label{0415_r1r2}
\end{figure}
...
Abbiamo quindi confutato l'ipotesi di Fowler\complementi{l'ipotesi di Fowler} e non solo: per la prima volta si osservò l'elettroscreening\index{elettroscreening}.
...
\paragraph{Il problema dello screening} Nonostante la nostra conquista su Fowler ci troviamo adesso ad affrontare un altro problema: l'effetto di elettroscreening\index{elettroscreening} dato dal modello $U_e = 220$ eV non era compatibile con quello misurato $U_e\simeq (294 \pm 47)$ eV! Si iniziò quindi a studiare tale fenomeno. Fu scelta la reazione (precedentemente non di interesse):
$$d + \ce{^3He} \to \ce{^4He} + p$$
...
$$p + d \to \ce{^3He} + \gamma$$
intorno al picco di Gamow. Questa reazione fa parte della catena $pp$\index{Catena protone-protone@Catena protone-protone $pp$}\footnote{Importante nelle proto-stelle: la contrazione innesca la $pp$, tuttavia quando si raggiungono temperature $T\sim \ord{6}$ keV il poco deuterio presente (dovuto all'arricchimento di popolazioni precedenti) può reagire e quindi può succedere che si inneschi un $d$-\textit{burning} prima di un H-\textit{burning}. Questo comporta un rallentamento della contrazione e quindi un aumento della vita media della proto-stella; le sue caratteristiche (luminosità, temperatura,\dots) rimangono \vir{congelate} fin quando dura il $d$-\textit{burning} e ciò avrà conseguenze su tali quantità all'innesco del H-\textit{burging}.\complementi{Evoluzione protostellare}}, ma è l'unico canale aperto per cui non influisce sul conto dei neutrini solari. 
...
\paragraph{Lo studio della he$\alpha$} Discutiamo adesso la reazione $\ce{^3He(\alpha,\gamma)\ce{^7Be}}$. Prima di LUNA erano stati sviluppati 2 metodi di misura:
\begin{enumerate}
...
\paragraph{ATOMKY \textit{Anomaly}}\index{ATOMKY Anomaly@ATOMKY \textit{Anomaly}} Per quanto riguarda la ricerca di nuova fisica acceleratori di maggiore potenza permetterebbero lo studio dell'anomalia di ATOMKY\esperimento{ATOMKI}, per la prima volta osservata con la reazione
$$p+ \ce{^7Li} \to \ce{^8Be}^* \to \ce{^8Be} + \gamma^* $$
$$\ce{^8Be}\to \alpha + \alpha \qquad\quad \gamma^* \to e^+ + e^-$$
Non fecero \textit{particles identification}, ovvero non misurarono direttamente gli elettroni e i positroni prodotti. L'anomalia che osservarono fu un picco nella sezione d'urto per un angolo $\theta_{ee} \sim 120^{\circ}$; si è quindi pensato alla possibilità che venisse prodotta una particella massiva sconosciuta (materia oscura) $\ce{^8Be}^* \to \ce{^8Be} + X$ e $X\to e^+ + e^- $ con $M_X\sim 17$ MeV, da cui il nome di particella X17\index{particella X17}.
...
ATOMKY si spinse più avanti\footnote{Anche nell'esperimento DAMA\esperimento{DAMA} agli LNGS dicono di aver osservato fluttuazioni di materia oscura, tuttavia solo loro riescono a ossevarle.} studiando:
...
Poniamoci nel centro di massa\footnote{Mettiamo $c=1$.}, dove vediamo $a$ e $b$ scontrarsi e $\gamma$ e $c$ \vir{\textit{scatterare}} con un certo angolo $\theta_{CM}$:
\begin{align*}
