...
\section{La prima reazione con \textit{ab-initio}}\label{0317-sec-abinitio}
Dopo il decadimento del neutrone, le prima reazione che avviene (secondo la Figura \ref{0315_net}) è:
$$p + n \to d + \gamma$$
...
$$d\sigma \overset{\text{Reg. d'Oro}}{=} \frac{\text{Probabilità di transizione}}{\text{Flusso incidente}} = \frac{W_{i\to f}}{v_{rel}}$$
...
$$W_{i\to f} = |V_{i\to f}|^2\frac{1}{T}\:dn \qquad \text{con   } dn = \frac{d^3q}{(2\pi)^3}\frac{dP_d}{(2\pi)^3} (2\pi)^3 \sigma^3(\vec{P}_d+\vec{q}) \Rightarrow \frac{d^3q}{(2\pi)^3}$$
...
\frac{d\sigma}{d\Omega_{\hat{q}}} &= \frac{e^2}{8\pi^2} |\widehat{\varepsilon}^+(\vec{q},\lambda) \cdot \vec{J}^+(\vec{q})|^2\, q dq \, \delta(E_p + E_n - m_d - \frac{q^2}{2m}-q) \frac{1}{v_{rel}}
\end{aligned}
\end{displaymath}
...
$$\frac{d\sigma}{d\Omega_{\hat{q}}} = \frac{e^2}{4\pi}\frac{1}{8\pi v_{rel}}\sum_{\lambda=\pm 1 ,\; \sigma_d = \pm 1, 0} \sum_{\sigma_n,\sigma_p = \pm \frac{1}{2}} \int q dq |\widehat{\varepsilon}^+(\vec{q},\lambda) \cdot \vec{J}^+(\vec{q})|^2\,\frac{\delta (q-\bar{q})}{1+\frac{\bar{q}}{2m_d}}$$
$$\frac{d\sigma}{d\Omega_{\hat{q}}}=\frac{e^2}{4\pi}\frac{1}{8\pi v_{rel}}\sum_{\lambda=\pm 1 ,\; \sigma_d = \pm 1, 0} \sum_{\sigma_n,\sigma_p = \pm \frac{1}{2}}  |\widehat{\varepsilon}^+(\vec{\bar{q}},\lambda) \cdot \vec{J}^+(\vec{\bar{q}})|^2\,\frac{\bar{q}}{1+\frac{\bar{q}}{m_d}}$$
dove abbiamo usato la proprietà della $\delta$\footnote{$$\delta (f(q)) = \sum_{i=0}^N\frac{\delta(q-\bar{q}_i)}{|f'(\bar{q}_i)|}$$
con $\bar{q}_i$ zeri della funzione $f$.}, per cui $\bar{q} = m_d \ppc{-1+ \sqrt{1+2\Delta E/m_d}}$ con $\Delta E = E_p + E_n - m_d \overset{\text{CM}}{=} m_n+m_p-m_d + T_{rel}$. Notiamo che questa espressione è simile a quella del decadimento $\gamma$, infatti questo formalismo\footnote{Ovvero potremo sempre scrivere ${d\sigma}/{d\Omega_{\hat{q}}} \propto 1/(8\pi v_{rel})\; \sum \sum \, |\dots|^2\, {\bar{q}}/{(1+\bar{q}/m_d)}$.} vale per ogni decadimento del tipo $a+b\to c +\gamma$.\\
...
=& \Bigl | \oss{\psi_{1\sigma_d}}{J_\lambda(\vec{q})}{\psi_{\sigma_p \sigma_n}(\vec{p})} \Bigr |^2
\end{aligned}
\end{displaymath}
per $psi_{\sigma_p \sigma_n}(\vec{p})$ ci aspetteremo dei multipoli come avevamo visto nel decadimento $\gamma$. Per $\widehat{\varepsilon}$ consideriamo polarizzazione circolare:
...
\begin{displaymath}
\begin{aligned}
\psi_{}(\vec{p}) &= 4\pi \sum_{S,S_z} \clebs{\frac{1}{2}\sigma_n,\,\frac{1}{2}\sigma_p}{S\,S_z}\, \sum_{L,Lz,\Lambda,\Lambda_z} \clebs{SS_z,\,LL_z}{\Lambda\,\Lambda_z} i^L \mathcal{Y}^*_{LL_z}(\widehat{p}) \, \psi_{np}^{(LS\Lambda\Lambda_z)} \\
%
\oss{\psi_{1\sigma_d}}{J_\lambda(\vec{q})}{\psi_{\sigma_p \sigma_n}(\vec{p})} &= 4\pi \,(\dots)\, \oss{\psi_{1\sigma_d}}{J_\lambda(\vec{q})}{\psi_{np}^{(LS\Lambda\Lambda_z)}}
\end{aligned}
\end{displaymath}
...
\begin{displaymath}
\begin{aligned}
\psi_{\sigma_p\sigma_n}(\vec{p}) &= 4\pi \clebs{\frac{1}{2}\sigma_p\,\frac{1}{2}\sigma_n}{00}\, \underbrace{\frac{1}{\sqrt{4\pi}}}_\text{Arm. sfer.} \, \psi_{np}(\ce{^1S_0}) \\
%
\oss{\psi_{1\sigma_d}}{J_\lambda(\vec{q})}{\psi_{\sigma_p \sigma_n}(\vec{p})} &\equiv \sqrt{4\pi} \clebs{\frac{1}{2}\sigma_p\,\frac{1}{2}\sigma_n}{00}\, j^{\ce{^1S_0}}_{\lambda\sigma_p} (\vec{q}) \\
j^{\ce{^1S_0}}_{\lambda\sigma_p} (\vec{q}) &\equiv \oss{\psi_{1\sigma_d}}{\int d^3x \; e^{-i\vec{q}\cdot\vec{x}}\widehat{\varepsilon}^+(\vec{q},\lambda) \cdot \vec{J}(\vec{x})}{\psi_{np}(\ce{^1S_0})} = \\
&= -\sqrt{2\pi} \sum_{\Lambda\geq 1} (-i)^\Lambda \sqrt{2\Lambda +1} \oss{\psi_{1\sigma_d}}{E_{\Lambda-\lambda}(q)  + \lambda \, M_{\Lambda-\lambda}(q)}{\psi_{np}(\ce{^1S_0})}
\end{aligned}
\end{displaymath}
dove abbiamo prima definito $j^{\ce{^1S_0}}_{\lambda\sigma_p} (\vec{q})$ e poi sviluppato in multipoli come nel decadimento $\gamma$.
...
\begin{displaymath}
\begin{aligned}
\sqrt{4\pi} \clebs{\frac{1}{2}\sigma_p\,\frac{1}{2}\sigma_n}{00}\, j^{\ce{^1S_0}}_{\lambda\sigma_p} (\vec{q}) &= \sqrt{4\pi} \clebs{\frac{1}{2}\sigma_p\,\frac{1}{2}\sigma_n}{00}\: \oss{\psi_{1\sigma_d}}{-\sqrt{2\pi}(-i) \sqrt{3} (\lambda\,M_{1-\lambda}(q))}{\psi_{np}(\ce{^1S_0})}=\\
&= i\sqrt{6\pi} \lambda \oss{\psi_{1\sigma_d}}{M_{1\lambda}}{\psi_{np}(\ce{^1S_0})} =\\
&= i \sqrt{6\pi} \lambda \:\underbrace{ \frac{\clebs{00,1\lambda}{1\sigma_d}}{\sqrt{1}}}_\text{Dal teo. di W-E} \: \underbrace{\bigl \langle \, \psi_{1\sigma_d}\,||\,M_1\,||\,\psi_{np}(\ce{^1S_0})}_\text{Reduced Matrix} = \\
&= i \sqrt{6\pi} \lambda \: \delta_{\lambda\sigma_d} \: |M_1(q)|
\end{aligned}
\end{displaymath}
...
$$\sum_{\sigma_n\sigma_p,\lambda\sigma_d}|\sqrt{4\pi} \clebs{\frac{1}{2}\sigma_p\,\frac{1}{2}\sigma_n}{00}\, j^{\ce{^1S_0}}_{\lambda\sigma_p} (\vec{q})|^2 = 4\pi \underbrace{\sum_{\sigma_n\sigma_p}|\clebs{\frac{1}{2}\sigma_p\,\frac{1}{2}\sigma_n}{00}|^2}_{=1} \; \sum_{\lambda\sigma_d} 6\pi \delta_{\lambda\sigma_d}\,|M_1|^2 = 48\pi^2 |M_1|^2$$
dove abbiamo usato $\sum_{\lambda\sigma_d}\delta_{\lambda\sigma_d}=\delta_{11} + \delta_{-1-1} =2$. Dobbiamo calcolare $M_1(q)$, ma dal momento che questo non dipende da $\lambda$ possiamo stimarlo per qualsiasi valore di $\lambda$ (per esempio $\lambda = +1$):
\begin{displaymath}
\begin{aligned}
...
A questo punto abbiamo allora bisogno della funzione d'onda del deutone, di quella di scattering $np$, del potenziale nucleare e di un metodo numerico che risolva l'equazione di \Sch{} sia per lo stato legato che per quello di scattering. Questo non è però sufficiente, è necessario anche un modello per la corrente elettromagnetica\footnote{Attenzione: in questo caso $J_i$ indica la corrente dell'$i$-esimo nucleone e non il momento angolare totale iniziale. Si invita il lettore da qui in poi a cercare di capire dal contesto il significato delle notazioni.} e ne avevamo uno:\index{corrente di convezione}
$$\vec{J}_i = \frac{1}{2m} \underbrace{\varepsilon_i \PPg{\vec{p}_i,\:e^{i\vec{q}\cdot\vec{r}_i}}}_\text{corrente di convezione} - \frac{i}{2m} \mu_i \, \vec{q}\times \vec{\sigma}_i\, e^{i\vec{q}\cdot\vec{r}_i}$$
$$\varepsilon \simeq \frac{1}{2}(1+\tau_{z,i})$$
...
Notiamo che $\zeta_{00}^+\, \mu_S\, \zeta_{01} = 0$ perché le due funzioni sono ortogonali fra loro, dunque sopravvive solo il pezzo con $\mu_V$ nella corrente:
...
Cambiamo variabili\footnote{$\vec{R}=(\vec{r}_1+\vec{r}_2)/2$ e $\vec{r}=\vec{r}_1-\vec{r}_2$.} per cui $\vec{r}_1 = \vec{R}+\vec{r}/2$ e $\vec{r}_2 = \vec{R}-\vec{r}/2$:
...
$$\sigma = \frac{\overbrace{4\pi\alpha}^{e^2}}{v_{rel}}\frac{\mu^2_V q^3}{4m^2} \Bigl |\int_0^{+\infty} dr \, u(r) u_S(r)\Bigr |^2$$
